...
describe "Key Management Service" do
  ...
  before :all do
    ...
    @test_key_ring_id = "#{@project_id}-test-#{Time.now.to_i}"
    @test_key_ring_location = "global"

    @test_key_ring = create_keyring project_id: @project_id,
                                    key_ring_id: @test_key_ring_id,
                                    location: @test_key_ring_location
  end
