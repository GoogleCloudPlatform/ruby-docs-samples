...
describe "metrics", :monitoring do
  ...
  let(:metric_name) {
    Google::Cloud::Monitoring::V3::MetricServiceClient.metric_descriptor_path project_id, metric_type
  }
  ...
  describe "write_time_series" do
    ...
    it "writes time series data that can be read back" do
      ...
      interval = Google::Monitoring::V3::TimeInterval.new start_time: start_time, end_time: end_time
      view = Google::Monitoring::V3::ListTimeSeriesRequest::TimeSeriesView::FULL

      received_time_series = retry_block 3 do
        response = client.list_time_series(project_path, filter, interval, view).to_a
